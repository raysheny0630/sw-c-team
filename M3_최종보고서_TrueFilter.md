...
| 집계 기준 | AI 로그 #3 확정 시점(6/14) 기준 — 기존 로그 11건 + AI 로그 #3 신규 10건(코드 정합화 리팩토링 건 포함), 공동 작성 건(PM·개발자)은 세션 단위 1건으로 집계 |
...
| 이름 | 역할 | 한 줄 소감 |
|------|------|------------|
| 김지우 | PM | 일정 관리는 계획표가 아니라 의존 관계 관리라는 것을 지연을 겪으며 배웠다. |
| 정주승 | 분석가 | 요구사항 ID 하나가 설계·구현·검토 문서 전체를 꿰는 실이 된다는 걸 체감했다. |
| 최라혜 | 설계자 | AI 초안에 태클을 거는 과정에서 SRP·OCP가 교과서 문장이 아니라 판단 기준이 되었다. |
| 박정우 | 개발자 | 403 오류 사흘이 어떤 강의보다 라이브러리 내부 동작을 깊이 가르쳐 줬다. |
| 주민찬 | QA/보안 | 결함을 찾는 것보다 "왜 미수정으로 남기는지"를 기록하는 일이 더 중요했다. |

---

*작성일: 2026-06-11 | 항목별 검토 반영·확정: 2026-06-14 | 작성: TrueFilter (C조) | 통합: 김지우 (PM)*
