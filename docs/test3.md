...
가이드 파일을 함께 첨부하니 `flowchart LR`, `subgraph`, `<<include>>`/`<<extend>>` 등 형식적인 부분을 별도로 지시하지 않아도 알아서 맞춰줬다. 뼈대를 잡는 시간이 크게 줄었고 팀이 내용 검토에 집중할 수 있었다.

**수정이 필요했던 이유**

AI는 형식은 잘 따랐지만 우리 프로젝트 고유의 표현(요구사항 정의서에서 쓴 이름)을 알지 못했다. 유스케이스 이름이 짧고 일상적인 표현으로 생성되어 공식 문서 느낌이 나지 않았다. 예외 처리도 구조는 맞지만 실제 동작 방식이 빠진 채 서술되어 팀이 직접 채워 넣었다.

**다음 작업에서 개선할 프롬프트 전략**

유스케이스 이름은 "요구사항 정의서의 FR 표현을 그대로 사용할 것"을 명시하면 수정 횟수를 줄일 수 있을 것 같다. 예외 처리는 "fallback 동작까지 구체적으로 서술할 것"을 조건으로 추가할 예정이다.
